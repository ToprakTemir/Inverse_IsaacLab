...
class ManagerBasedRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        if self._reset_blocked_ids is not None:
            # if some envs are blocked from resetting, we need to remove them from the reset list
            reset_env_ids = torch.tensor(
                [eid for eid in reset_env_ids.cpu().numpy() if eid not in self._reset_blocked_ids],
                device=self.device,
                dtype=torch.long,
            )
